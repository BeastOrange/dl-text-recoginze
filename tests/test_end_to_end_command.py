...
def test_evaluate_end2end_image_mode_accepts_unified_checkpoint(
    monkeypatch,
    tmp_path: Path,
) -> None:
    (tmp_path / "PLAN.md").write_text("plan", encoding="utf-8")
    image_path = tmp_path / "scene.png"
    image_path.write_bytes(b"fake")
    unified_checkpoint = tmp_path / "end2end_best.pt"
    unified_checkpoint.write_bytes(b"pt")
    ...
    assert exit_code == 0
    assert called["end2end_checkpoint"] == unified_checkpoint
    assert called["detector_checkpoint"] is None
    assert called["recognizer_checkpoint"] is None
