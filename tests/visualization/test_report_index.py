...
def test_build_training_report_index_filters_legacy_semantic_reports(tmp_path: Path) -> None:
    train_dir = tmp_path / "train"
    train_dir.mkdir(parents=True)
    for name in (
        "detection_summary.md",
    ...
    content = output.read_text(encoding="utf-8")
    assert "detection_summary.md" in content
    assert "recognition_summary.md" in content
    assert "semantic_summary.md" not in content
    assert "semantic_ablation_template.md" not in content
