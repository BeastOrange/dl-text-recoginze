...
def test_train_recognizer_returns_nonzero_for_removed_legacy_model(tmp_path, monkeypatch) -> None:
    ...
    config_path = tmp_path / "configs" / "recognition" / "legacy.yaml"
    config_path.parent.mkdir(parents=True, exist_ok=True)
    config_path.write_text(
        "\n".join(
            [
                "experiment_name: legacy_model_smoke",
                ...
                "output_dir: artifacts/checkpoints/recognition/legacy_model",
                "epochs: 1",
                "batch_size: 2",
                "image_height: 48",
                "image_width: 320",
...
def test_train_recognizer_transformer_runs_smoke(tmp_path, monkeypatch) -> None:
    (tmp_path / "PLAN.md").write_text("plan", encoding="utf-8")
    ...
    config_path.write_text(
        "\n".join(
            [
                "experiment_name: transformer_smoke_cmd",
                "model_name: transformer",
                "dataset_manifest: data/processed/recognition_splits/train.jsonl",
                "validation_manifest: data/processed/recognition_splits/val.jsonl",
                "charset_file: data/processed/charset.txt",
                "output_dir: artifacts/checkpoints/recognition/transformer_smoke_cmd",
                "epochs: 1",
                "batch_size: 2",
                "image_height: 32",
                "image_width: 128",
